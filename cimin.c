...
void strforcecpy(char* to, char* from, int len){        //force to copy even if null character
    for(int i=0; i<len; i++){
        to[i]=from[i];
    }
}
...
bool ExecutePrgm(char* crash, char* prgm, char** param, char* errmsg){
    ...
    if(child<0) {
        ...
        int sum=0;
        while((s=read(pipes[0],buf,128))){	//read returns 0 if file is closed
            sum+=s;
            //out[sum]=0x0;
            strncat(out,buf,128);
            out[sum]=0x0;
...
char* Reduce(char* crashInput, char* prgm, char** param, char* errmsg){
    ...
    while(s > 0){
        ...
        for(int i = 0; i < len-s+1; i++){
            ...
            //strncpy(head,crashInput,i);
            strforcecpy(head,crashInput,i);
            head[i] = '\0';

            //strncpy(tail,crashInput+i+s,len-s-i);
            strforcecpy(tail, crashInput+i+s,len-s-i);
            tail[len-s-i]='\0';
        ...
        for(int i = 0; i < len - s ;i++){
            ...
            strforcecpy(mid, crashInput+i,s);
            mid[s] = '\0';
...
int main(int argc, char* argv[]) {
    ...
    strforcecpy(result,Reduce(crashInput,inputs[3],tarArg,inputs[1]),crlen);
